...
import { Building2, DoorOpen, Users, CreditCard, TrendingUp, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="page-subtitle">
          {isSuperAdmin ? 'System-wide overview' : `${user?.branch?.name || 'Branch'} overview`}
          {' · '}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {isSuperAdmin && (
          <StatCard label="Total Branches" value={summary.total_branches} icon={Building2} iconBg="bg-purple-100" iconColor="text-purple-600" />
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card">
          ...
          <div className="card-body">
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={room_breakdown} dataKey="count" nameKey="status" cx="50%" cy="50%" outerRadius={75} label={({ status, count }) => `${count}`}>
                  ...
                  {unpaid_tenants.map((t, i) => (
                    <tr key={i}>
                      ...
                      <td className="text-xs">
                        {t.room_number}
                        {isSuperAdmin && <div className="text-gray-400 text-xs">{t.branch_name}</div>}
                      </td>

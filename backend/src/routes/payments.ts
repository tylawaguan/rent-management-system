...
router.get('/', (req: Request, res: Response) => {
  ...
  let query = `SELECT p.*, t.first_name || ' ' || t.last_name as tenant_name, t.phone as tenant_phone,
  ...
  if (req.user!.role !== 'super_admin') {
    query += ' AND p.branch_id = ?'; params.push(req.user!.branch_id);
  } else if (branch_id) {
    query += ' AND p.branch_id = ?'; params.push(branch_id);
  }
  if (tenant_id) { query += ' AND p.tenant_id = ?'; params.push(tenant_id); }
  if (status) { query += ' AND p.status = ?'; params.push(status); }
  if (month_year) { query += ' AND p.month_year = ?'; params.push(month_year); }
...
// Record payment with optional receipt file upload
router.post(
  '/',
  ...
  (req: Request, res: Response) => {
    ...
    db.prepare(`INSERT INTO payments (id, tenant_id, branch_id, room_id, amount, amount_due, penalty, payment_type,
    ...
    if (paidAmount > 0) {
      db.prepare(`INSERT INTO notifications (id, tenant_id, branch_id, type, channel, title, message) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
...
router.get('/:id', (req: Request, res: Response) => {
  const db = getDb();
  const payment = db.prepare(`SELECT p.*, t.first_name || ' ' || t.last_name as tenant_name, t.phone as tenant_phone,
    r.room_number, b.name as branch_name FROM payments p
    LEFT JOIN tenants t ON p.tenant_id=t.id LEFT JOIN rooms r ON p.room_id=r.id LEFT JOIN branches b ON p.branch_id=b.id
    WHERE p.id=?`).get(req.params.id) as any;
  if (!payment) return res.status(404).json({ error: 'Payment not found' });
  if (req.user!.role !== 'super_admin' && req.user!.branch_id !== payment.branch_id) return res.status(403).json({ error: 'Access denied' });
  res.json(payment);
});

router.put('/:id', authorize('super_admin', 'admin', 'manager', 'accountant'), upload.single('receipt_file'), (req: Request, res: Response) => {
  const db = getDb();
  ...
  db.prepare(`UPDATE payments SET amount=?, penalty=COALESCE(?,penalty), payment_method=COALESCE(?,payment_method),
    paid_date=COALESCE(?,paid_date), status=?, receipt_number=?, receipt_file=COALESCE(?,receipt_file),
...
router.post('/generate-monthly', authorize('super_admin', 'admin'), (req: Request, res: Response) => {
  const db = getDb();
  ...
  let query = `SELECT * FROM tenants WHERE status='active'`;
  const params: any[] = [];
  if (effectiveBranch) { query += ' AND branch_id=?'; params.push(effectiveBranch); }

  const tenants = db.prepare(query).all(...params) as any[];
  let created = 0;

  for (const tenant of tenants) {
    const exists = db.prepare('SELECT id FROM payments WHERE tenant_id=? AND month_year=? AND payment_type=?').get(tenant.id, month_year, 'rent');
    if (!exists) {
      db.prepare(`INSERT INTO payments (id, tenant_id, branch_id, room_id, amount, amount_due, payment_type, due_date, month_year, status, recorded_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
